...
impl Go2Rtc {
    ...
    fn write_config(&self, cameras: &[Camera]) -> Result<()> {
        ...
        // attack surface. `origin: "*"` then lets our same-machine web UI open
        // the player's WebSocket cross-origin (UI on :8080 → go2rtc on :1984);
        // without it go2rtc rejects the handshake with 403. (RTSP/WebRTC keep
        // their own listeners for the recorder and browser media transport.)
        let mut yaml = format!(
