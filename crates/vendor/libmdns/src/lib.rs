// CAMMY PATCH: vendored third-party code — keep upstream style, silence lints.
...
impl CommandSender {
    fn send(&mut self, cmd: Command) {
        for tx in self.0.iter_mut() {
            // CAMMY PATCH: never panic when the responder FSM task is already
            // gone. The FSM future is polled by the caller's runtime and can be
            // dropped before the Responder/Service handles (their Drop impls
            // send Shutdown/Unregister through here); with the upstream
            // .expect(), that ordering panics inside Drop during unwinding =>
            // process ABORT on every HAP bridge teardown. A dead receiver at
            // shutdown is fine - there is nothing left to notify.
