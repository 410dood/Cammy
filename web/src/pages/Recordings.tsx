...
const tsToLocalInput = (ts: number) => {
  const d = new Date(ts * 1000);
  d.setMinutes(d.getMinutes() - d.getTimezoneOffset());
  return d.toISOString().slice(0, 16);
};
const localInputToTs = (v: string) => Math.floor(new Date(v).getTime() / 1000);
 ...
 *  warns you. */
function ExportRangeCard({ cameraId, defaultFrom }: { cameraId: number; defaultFrom: number }) {
